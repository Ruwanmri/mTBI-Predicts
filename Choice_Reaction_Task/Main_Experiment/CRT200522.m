...
cfgCue = cue_properties(cfgScreen, cfgCue);  % destination rectangle to present thse cue
...
nstim = 0;  % count number of stimuli in total (ruwan testing)

for n= 1:cfgExp.numruns
...
for blk = 1:cfgExp.numBlock
    ...
    WaitSecs(20)  % wait for triggers to reset
    ...
    if blk ~= cfgExp.numBlock
        %cfgOutput = draw_myText(cfgScreen, cfgExp, cfgTxt.breakTxt, cfgTxt, cfgOutput, cfgTrigger, cfgFile, cfgEyelink, cfgCue);%original
        %WaitSecs(5);
    end
end
cfgOutput = draw_myText(cfgScreen, cfgExp, cfgTxt.ratingTxt, cfgTxt, cfgOutput, cfgTrigger, cfgFile, cfgEyelink, cfgCue);%ruwan testing
end
